...
func TestApplyDataDirCreatesCurrentSchemaAndIsIdempotent(t *testing.T) {
	...
	if first.LatestVersion != "0033" {
		t.Fatalf("latest version = %q, want 0033", first.LatestVersion)
	}
...
func TestCheckDataDirReadsCommittedWALState(t *testing.T) {
	...
	if _, err := db.Exec(`UPDATE schema_migrations SET dirty = 1 WHERE version = '0033'`); err != nil {
		t.Fatalf("mark migration dirty: %v", err)
	}
	if _, err := db.Exec(`PRAGMA wal_checkpoint(TRUNCATE)`); err != nil {
		t.Fatalf("checkpoint dirty state: %v", err)
	}
	if _, err := db.Exec(`UPDATE schema_migrations SET dirty = 0 WHERE version = '0033'`); err != nil {
