...
The command menu and subcommand help are generated by Cobra. Run `mnemo --help`
or `mnemo <command> --help` for the authoritative interface available in the
installed binary.
...
mnemo session project-obs-count <id> Count project-scoped observations for a session
